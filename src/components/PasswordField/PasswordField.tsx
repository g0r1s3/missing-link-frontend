...
export default function PasswordField({
  ...
  const inputId =
    id || (label ? `pf-${label.replace(/\s+/g, '-').toLowerCase()}` : undefined)

  // kebab-case modifier: pf-input-error
  const cls = ['pf-input', error ? 'pf-input-error' : '', className]
    .filter(Boolean)
    .join(' ')
  ...
  return (
    <label className="pf-root" htmlFor={inputId}>
      ...
      {(helperText || error) && (
        // kebab-case modifier: pf-helper-error
